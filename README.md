...
- **설정 섹션**: 기본값·상한·계산식이 전부 앱 코드에서 온다 — `app/settings.tsx`(상한·미리보기 식·추세 고정값 안내), `lib/appSettings.ts`(`DEFAULT_APP_SETTINGS`), `features/scalper/autopilot.ts`(`MAX_GRIDS_LIMIT`, `TREND_CONFIG.stopLossPct`), `core/trend/signal.ts`(진입·청산 규칙), `core/ranking/index.ts`(순위 원천 카탈로그·총 상한). **앱에서 이 값들이 바뀌면 여기도 같이 고쳐야 한다.** 시뮬레이터의 첫 진입 금액은 앱과 같은 `진입금액 × 동시 종목 수`(수량 모드면 `수량 × 현재가`)라 앱 화면의 미리보기 숫자와 일치해야 한다.
- **전략 변경 이력**: 2026-08-18에 변곡점(Savitzky–Golay)+물타기 그리드에서 추세(3분봉 4선 상승 진입 · 종가<5선 청산 · −7% 손절, 물타기 없음)로 바뀌었다. 그때 물타기 전용이던 `grid-risk.html`(그리드 리스크 계산표)과 호가 탭 스크린샷은 지웠다 — 앱이 `TREND_MODE=false`로 롤백되면 git 이력에서 되살린다.
